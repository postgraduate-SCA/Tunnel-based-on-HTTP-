...
static int tunnel_write_request(Tunnel *tunnel,char request,void *data,u_short length)
{
	...
	if(data!=NULL)
	{
		...
		if(tunnel_write_data(tunnel,(char *)&network_length,sizeof(network_length))==-1)
			return -1;
	                              ...
	                              char *buf, int *length)
{
	...
	u_short len,llen;
	int n;
	//printf("tunnel_read_request 开始读取数据!\n");
	n = recv (tunnel->in_fd, tmp, sizeof(char), 0);
	...
	*length = llen;
	...
	if ( llen > 0 )
	{
		n =read_all ( tunnel->in_fd, buf , llen );
		if( n <= 0) return -1;
		//printf("数据:%s",buf);
		//tunnel->in_total_raw  += n;
    }
